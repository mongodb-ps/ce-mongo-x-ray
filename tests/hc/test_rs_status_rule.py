...
def test_no_primary():
    ...
    result = rule.apply(RS_STATUS_NO_PRIMARY, result_template={"host": "cluster"})
...
def test_unhealthy_member():
    ...
    result = rule.apply(RS_UNHEALTHY_MEMBER, result_template={"host": "cluster"})
    assert result is not None
...
def test_initializing_member():
    ...
    result = rule.apply(RS_INITIALIZING_MEMBER, result_template={"host": "cluster"})
    assert result is not None
...
def test_lagged_member():
    ...
    result = rule.apply(RS_STATUS_LAGGED_MEMBER, result_template={"host": "cluster"})
    assert result is not None
    assert len(result) == 2
...
def test_normal_status():
    ...
    result = rule.apply(RS_STATUS_NORMAL, result_template={"host": "cluster"})
    assert result is not None
    assert len(result) == 0
