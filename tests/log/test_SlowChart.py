...
def test_slow_chart_item():
    item = SlowChartItem(output_folder="/tmp", config={})
    ...
    assert len(output) == 4
    result = output[0]
    assert result["attr"]["ns"] == "Restaurant.pizzas"
    assert result["attr"]["durationMillis"] == 0

    result = output[2]
    assert result["attr"]["ns"] == "admin.$cmd"
    assert result["attr"]["durationMillis"] == 0
